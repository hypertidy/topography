...
read_rema_100m <- function(xylim = NULL, ...) {
  .tp_standard_version_check()
  ext <- topography:::.tp_rema_defaultgrid(xylim)

  vfile <- topography::topography_source("rema")
  prj <- ext@crs@projargs
  if (is.na(prj) || nchar(prj) == 0) stop("no valid projection metadata on 'xylim', this must be present")
  l <- vapour::vapour_warp_raster(vfile, band = 1, extent = raster::extent(ext), dimension = dim(ext)[2:1],
                                  wkt = vapour::vapour_srs_wkt(raster::projection(ext)),
                                  resample = "bilinear")
  raster::setValues(raster::brick(ext, ext, ext), matrix(unlist(l), ncol = 1))
}
